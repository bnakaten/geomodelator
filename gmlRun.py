...
class Compress:
    def __init__(self, rs):
        ...
        baseDir = rs['config']['base']+ '/' if rs['config']['base'] != "" else ''
        inputDir = \
            rs['config']['inputPath']+ '/' if rs['config']['inputPath'] != "" else ''
        outputDir = \
            rs['config']['outputPath'] + '/' if rs['config']['outputPath'] != "" else ''

        zfile = 'Model.zip'
        zf = zipfile.ZipFile(baseDir + outputDir + zfile, mode='w')
        ...
        for i, file in enumerate(rs['structure']['file']):
            if rs['structure']['extension'][i] == 'csv' or \
                ...
                zf.write(baseDir + outputDir + filename, filename, compress_type=compression)
        ...
        zf.write(baseDir + filename, filename, compress_type=compression)

        fileExtension = rs['config']['file'].split('.')[-1]

        filename = rs['config']['file'].split('.')[0] + (
            '_out.yaml' if fileExtension == 'json' else '_out.json'
        )
        ...
        for filename in rs['structure']['file']:
            zf.write(baseDir + inputDir + filename, filename, compress_type=compression)
